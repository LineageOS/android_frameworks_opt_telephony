...
public class SonyQualcommRIL extends QualcommSharedRIL implements CommandsInterface {
    ...
    responseIccCardStatus(Parcel p) {
        ...
        updateIccType(status, numApplications);
        return status;
    }

    private void updateIccType (IccCardStatus status, int numApplications) {
        int appType;
        if (numApplications > 0) {
            if (mPhoneType == RILConstants.CDMA_PHONE)
                appType = status.mCdmaSubscriptionAppIndex;
            else
                appType = status.mGsmUmtsSubscriptionAppIndex;

            IccCardApplicationStatus application = status.mApplications[appType];
