        ...
        if (ccm != null) {
            ...
            if (cc != null) {
                if (cc.getBoolean("sprint_mwi_quirk")) {
                    ...
                    int voicemails = PreferenceManager.getDefaultSharedPreferences(context)
                            .getInt("sprint_vm_count_" + phone.getSubId(), 0);
                    phone.setVoiceMessageCount(voicemails);
                }
            }
        }
    }
        ...
        if (mSprintMwiQuirk) {
            ...
            if (address != null && address.equals("9016")) {
                ...
                if (message != null) {
                    ...
                    //                 TODO: Check whether the offset changes to 7 hours
                    //                 (PDT timezone) during Daylight Savings Time
                    ...
                    //              have seemingly no pattern or significance
                    ...
                    if (sprintVvmMatcher.matches()) {
                        int voicemails = PreferenceManager.getDefaultSharedPreferences(mContext)
                                .getInt("sprint_vm_count_" + mPhone.getSubId(), 0);

                        if (sprintVvmMatcher.group(3).equals("400") && voicemails < 99) {
                            voicemails++;
                        } else if (sprintVvmMatcher.group(3).equals("41f") && voicemails > 0) {
                            voicemails--;
                        }

                        mPhone.setVoiceMessageCount(voicemails);
                        PreferenceManager.getDefaultSharedPreferences(mContext).edit()
                                .putInt("sprint_vm_count_" + mPhone.getSubId(), voicemails).apply();
                        return Intents.RESULT_SMS_HANDLED;
                    }
                }
            }
