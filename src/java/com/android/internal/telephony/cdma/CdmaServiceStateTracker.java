...
public class CdmaServiceStateTracker extends ServiceStateTracker {
    ...
    protected void updateSpnDisplay() {
        ...
        if (combinedRegState == ServiceState.STATE_OUT_OF_SERVICE) {
            ...
            // Check numeric to spn lookup table
            String spnOverride = Operators.operatorReplace(mSS.getOperatorNumeric());
            //If the entry exists, the lookup returns the mapped spn name,
            // otherwise returns the originally provided numeric
            boolean isValidOpNumeric = !TextUtils.equals(spnOverride, mSS.getOperatorNumeric());
            if (isValidOpNumeric && (TextUtils.isEmpty(mSS.getOperatorAlphaLong())
                    || TextUtils.equals(mSS.getOperatorAlphaLong(), mSS.getOperatorNumeric()))) {
                // If modem reports a valid numeric, but the operator name is not set
                // or erroneously set to numeric, override
                plmn = spnOverride;
            } else if (isInHomeList(opNumArray, ruimNumeric)
                    || isInHomeList(opNumArray, mSS.getOperatorNumeric())
                    || TextUtils.equals(mSS.getOperatorNumeric(), opNum)
                    || TextUtils.isEmpty(mSS.getOperatorAlphaLong())
                    || TextUtils.equals(mSS.getOperatorAlphaLong(), mSS.getOperatorNumeric())) {
                // Use system property (or fallback to Ruim SPN) if one of the following  is true
                ...
                // -modem reported numeric is the same as the primary home numeric property
                // -modem reported plmn is null or empty
                // -modem reported plmn is erroneously set to numeric
                plmn = operator;
            } else {
    ...
    */
     ...
     * @return true if the value is found; false if list or numeric is null,
     * or numeric is not found in the list
     */
