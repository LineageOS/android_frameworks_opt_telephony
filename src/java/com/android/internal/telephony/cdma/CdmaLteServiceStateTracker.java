...
public class CdmaLteServiceStateTracker extends CdmaServiceStateTracker {
    ...
    protected void pollStateDone() {
        ...
        boolean hasRegistered = ss.getVoiceState() != ServiceState.STATE_IN_SERVICE
                && newSS.getVoiceState() == ServiceState.STATE_IN_SERVICE;

        boolean hasDeregistered = ss.getVoiceState() == ServiceState.STATE_IN_SERVICE
                && newSS.getVoiceState() != ServiceState.STATE_IN_SERVICE;
        ...
        if (ss.getVoiceState() != newSS.getVoiceState()
                || ss.getDataRegState() != newSS.getDataRegState()) {
            EventLog.writeEvent(EventLogTags.CDMA_SERVICE_STATE_CHANGE, ss.getVoiceState(),
                    ss.getDataRegState(), newSS.getVoiceState(), newSS.getDataRegState());
        }
        ...
        if (hasChanged) {
            if (phone.isEriFileLoaded()) {
                ...
                } else if (ss.getVoiceState() == ServiceState.STATE_POWER_OFF) {
        ...
        synchronized(mCellInfo) {
            ...
            if (hasRegistered || hasDeregistered || cidChanged) {
                ...
                boolean registered = ss.getVoiceState() == ServiceState.STATE_IN_SERVICE;
