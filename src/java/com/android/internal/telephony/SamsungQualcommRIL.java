...
public class SamsungQualcommRIL extends RIL implements CommandsInterface {
    ...
    protected Object responseSignalStrength(Parcel p) {
        ...
        if (response[7] == 99) {
            ...
            response[7] &= 0xff;
            /* cdma lte sleep fix: sometimes lte sleeps but  it shows asu 99  but lte is enabled, so force lte to gsm/cdma signal strength */
            if (response[7]==99){
                if(!isGSM){
                    response[8] = response[2];
                    // force it to not be invalid or unknown set asu to 0
                    response[7]=0;
                }
                else
                    response[7] = response[0];
            }
        }
