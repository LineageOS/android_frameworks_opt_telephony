...
public class DctController extends Handler {
    ...
    protected DctController(PhoneProxy[] phones) {
        ...
        mRequestInfos = new ArrayList<HashMap<Integer, RequestInfo>>();
    ...
    private int releaseNetwork(NetworkRequest request) {
        for (int i = 0; i < mPhoneNum; i++) {
            RequestInfo requestInfo = mRequestInfos.get(i).get(request.requestId);
            ...
            mRequestInfos.get(i).remove(request.requestId);
    ...
    private void onExecuteRequest(RequestInfo requestInfo) {
        ...
        for (int i = 0; i < mPhoneNum; i++) {
            if (mRequestInfos.get(i).containsKey(requestInfo.request.requestId)) {
                validRequest = true;
                break;
            }
        }
