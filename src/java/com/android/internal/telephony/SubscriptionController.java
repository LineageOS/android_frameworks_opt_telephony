...
public class SubscriptionController extends ISub.Stub {
    ...
    private void setDefaultDataSubNetworkType(int subId) {
        ...
        if (isDsds && !isMultiRat && len > 1) {
            ...
            int networkType2 = 0;
            int phoneId1 = 0;
            int phoneId2 = 0; // TODO: Better defaults for all 4
            for (int phoneId = 0; phoneId < len; phoneId++) {
                ...
                if (id == subId) {
                    try {
                        networkType1 = TelephonyManager.getIntAtIndex(mContext.getContentResolver(),
                                    Settings.Global.CONFIGURED_NETWORK_MODE, phoneId);
                    } catch (SettingNotFoundException snfe) {
                        networkType1 = PhoneFactory.calculatePreferredNetworkType(mContext, id);
                    }
                    phoneId1 = phoneId;
                    logdl("[setDefaultDataSubNetworkType] networkType1: " + networkType1 + ", phoneId1: " + phoneId1);
                } else {
                    networkType2 = Phone.NT_MODE_GSM_ONLY;
                    phoneId2 = phoneId;
                    logdl("[setDefaultDataSubNetworkType] networkType2: " + networkType2 + ", phoneId2: " + phoneId2);
                }
            }
            ModemBindingPolicyHandler.getInstance().setDsdsNetworkType(networkType1, phoneId1, networkType2, phoneId2);
        }
    }
