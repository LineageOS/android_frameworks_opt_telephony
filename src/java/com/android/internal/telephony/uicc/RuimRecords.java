...
public final class RuimRecords extends IccRecords {
    ...
    public int getDisplayRule(String plmnNumeric) {
        ...
        if ((mContext != null) && mContext.getResources().getBoolean(
        ...
        } else if (mParentApp != null && mParentApp.getUiccCard() != null && mParentApp.getUiccCard().getOperatorBrandOverride() != null) {
            ...
            if (mCsimSpnDisplayCondition && !TextUtils.isEmpty(getServiceProviderName())) {
                // check CSIM SPN Display Condition (applicable on home network), but only if SPN was found on this RUIM
                rule = SPN_RULE_SHOW_SPN;
            } else {
                // CSIM SPN Display does not require a SPN display, or SPN not found on RUIM, then revert to currently registered network
                rule = SPN_RULE_SHOW_PLMN;
            }
        } else {
    ...
    /**
    * Checks if current registered PLMN is home PLMN (PLMN numeric matches the one reported in CSIM)
    */
